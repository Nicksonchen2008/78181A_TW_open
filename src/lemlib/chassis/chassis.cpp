...
void lemlib::Chassis::swingTo(float heading, bool side, int timeout, bool forwards, float maxSpeed, bool async) {
    // take the mutex
    mutex.take(TIMEOUT_MAX);
    // if the function is async, run it in a new task
    if (async) {
        pros::Task task([&]() { swingTo(heading, side, timeout, forwards, maxSpeed, false); });
        mutex.give();
        pros::delay(10); // delay to give the task time to start
        return;
    }
    ...
    while (pros::competition::get_status() == compState && !pid.settled()) {
        ...
        if(side==false){
            // drivetrain.leftMotors->set_brake_modes(MOTOR_BRAKE_BRAKE);
...
void lemlib::Chassis::turnToAngle(float heading, bool side, int timeout, bool forwards, float maxSpeed, bool async) {
    // take the mutex
    mutex.take(TIMEOUT_MAX);
    // if the function is async, run it in a new task
    if (async) {
        pros::Task task([&]() { turnToAngle(heading, side, timeout, forwards, maxSpeed, false); });
        mutex.give();
        pros::delay(10); // delay to give the task time to start
        return;
    }
